            ...
            do j=js_obc+1,je_obc
              ...
              if(OBC%add_tidal_harmonics) then
                tidal_vel = US%m_s_to_L_T*segment%field(segment%uamp_index)%buffer_dst(I,j,1) * &
                  cos(TWOPI*(time_type_to_real(Time) - time_type_to_real(set_date(1981,1,1)))/OBC%tidal_periods(1) - segment%field(segment%uphase_index)%buffer_dst(I,j,1))
              else
                tidal_vel = 0.0
              endif
              do k=1,G%ke
            ...
            do i=is_obc+1,ie_obc
              ...
              if(OBC%add_tidal_harmonics) then
                tidal_vel = US%m_s_to_L_T*segment%field(segment%vamp_index)%buffer_dst(I,j,1) * &
                  cos(TWOPI*(time_type_to_real(Time) - time_type_to_real(set_date(1981,1,1)))/OBC%tidal_periods(1) - segment%field(segment%vphase_index)%buffer_dst(I,j,1))
              else
                tidal_vel = 0.0
              endif
              do k=1,G%ke
            ...
            do i=is_obc2,ie_obc
              if(OBC%add_tidal_harmonics) then
                tidal_elev = segment%field(segment%zamp_index)%buffer_dst(i,j,1) * &
                  cos(TWOPI*(time_type_to_real(Time) - time_type_to_real(set_date(1981,1,1)))/OBC%tidal_periods(1) - segment%field(segment%zphase_index)%buffer_dst(i,j,1))
              else
                tidal_elev = 0.0
              endif
